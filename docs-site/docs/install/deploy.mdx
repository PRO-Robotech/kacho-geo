...
таблицу `geo_outbox` с LISTEN/NOTIFY-триггером; последующие добавляют `CHECK` на `zones.status`
и таблицу `operations` (LRO admin-мутаций). Применяются **отдельным** бинарём
