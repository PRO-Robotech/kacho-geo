...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>operationresolver</strong></td><td><code>internal/operationresolver/</code></td><td>Доменный resolver осиротевших LRO поверх read-портов repo (сверка операции с committed-реальностью)</td></tr>
...
инварианты выражены на уровне БД (FK / PK / CHECK), а не software-проверками. Схему накатывают
goose-миграции: baseline `0001_initial` (таблицы `regions`, `zones`, `geo_outbox`), затем
`CHECK` на `zones.status` и таблица `operations` (LRO admin-мутаций).
...
erDiagram
  ...
  operations {
    text id PK "prefix geo + crockford"
    bool done
    bytea metadata_data "Any (Create/Update/Delete<R>Metadata)"
    bytea response_data "Region | Zone | Empty"
    integer error_code
    text principal_id "владелец операции"
  }
```
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>operations</code></td><td>LRO admin-мутаций (структура — corelib <code>operations.Repo</code>)</td><td><code>id</code> TEXT PK (префикс <code>geo</code>); <code>principal_*</code> — владелец; <code>account_id</code> всегда NULL (каталог cluster-scoped)</td></tr>
  </tbody>
</table>
...
## Async LRO admin-мутации

Admin-мутации Region/Zone следуют стандартной async-модели Kachō (мутации → `Operation`). RPC
`Internal*Service.Create/Update/Delete` синхронно валидирует формат id, пишет строку `operations`
(`done=false`) и запускает worker-горутину (движок — `kacho-corelib/operations`); клиент сразу
получает квитанцию-`Operation`. Worker выполняет доменную запись в БД и финализирует строку
(`done=true`, `response=Region/Zone/Empty` либо `error=google.rpc.Status`). Клиент поллит
`OperationService.Get(id)` до `done=true`.

- **Владелец операции** — принципал, создавший её (`principal_*` строки). `OperationService`
  помечен exempt от per-RPC ReBAC-Check; вместо этого `GetOwned` / `CancelOwned` энфорсят
  ownership-предикат **в SQL** (within-service инвариант, без software-TOCTOU). Чужой /
  несуществующий id → одинаковый `NOT_FOUND` (no existence-oracle).
- **Durable-восстановление**: если сервис упал между записью `Operation` и финализацией, orphan
  добирает reconciler (corelib-движок + доменный `operationresolver` kacho-geo) — сверяет
  осиротевшую операцию с committed-реальностью ресурса и приводит статус в соответствие (не
  re-drive worker'а).

Полное описание модели, полей и REST — [Operations (LRO)](/api/operations).

## Audit-outbox в одной транзакции

Admin-мутации (Create / Update / Delete Region/Zone) атомарно пишут строку в `geo_outbox` **в
той же writer-транзакции**, что и доменная запись (в worker'е операции). Так аудит не теряется:
либо изменение и его запись в журнал коммитятся вместе, либо откатываются вместе. Это конвенция
Kachō (`<domain>_outbox`). Триггер на INSERT шлёт `pg_notify` — подписчики узнают о новых
событиях без поллинга.
