...
каталога — синхронное; admin-мутации Region/Zone — асинхронные, возвращают `Operation`
(поллинг `OperationService.Get`, см. ниже).
...
<div className={hero.featureGrid}>
  ...
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>▤</span>
    <span className={hero.featureTitle}>Sync read · async admin</span>
    <span className={hero.featureBody}>Чтение каталога синхронное; admin-мутации Region/Zone — async <code>Operation</code> (поллинг <code>OperationService.Get</code>).</span>
  </div>
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>⛓</span>
